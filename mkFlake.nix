  ...
  coreModule =
    ...
    let
        ...
        let
          ...
          instantiate =
            if enriched.raw.instantiate != null then
              ...
              config.targets.${enriched.target}.instantiate
                or (throw "flakey: node '${name}' has undefined target '${enriched.target}'");
        in
        if errors != [ ] then
          throw (
            "flakey: configuration errors in node '${name}':\n"
        ...
        let
          ...
          formatModule =
            ...
            let
              ...
              requiresStr = lib.optionalString (
                m.requires != [ ]
              ) " (requires: ${lib.concatStringsSep ", " m.requires})";
              optsStr =
                if paths == [ ] then
                  "  (no options)"
                else
      ...
      allOptionsApp = lib.genAttrs config.systems (system: {
        type = "app";
        program = toString (
          nixpkgs.legacyPackages.${system}.writeShellScript "flakey-options" ''
            cat <<'EOF'
            # flakey modules
      ...
      checkOptionsApp = lib.genAttrs config.systems (system: {
        type = "app";
        program = toString (
          nixpkgs.legacyPackages.${system}.writeShellScript "flakey-check" ''
            cat <<'EOF'
            ${optionsCheck}
            EOF
      ...
      allNodesApp = lib.genAttrs config.systems (system: {
        type = "app";
        program = toString (
          nixpkgs.legacyPackages.${system}.writeShellScript "flakey-nodes" ''
            cat <<'EOF'
            # flakey nodes
